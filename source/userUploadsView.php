...
if(isset($_GET['content_id']) && isset($_GET['media_type'])) {
    ...
    mysqli_stmt_execute($stmt);
    $uploadssearch = mysqli_stmt_get_result($stmt) or die('Failed to search uploads');
    mysqli_stmt_close($stmt);
	while($uploadssearchresults = mysqli_fetch_array($uploadssearch)) {
		$contentids[] = $uploadssearchresults["CONTENT_ID"];
		$contenttitles[] = $uploadssearchresults["CONTENT_TITLE"];
	}
	...
	if($canview) {
        $stmt = mysqli_prepare($dbconnection, "UPDATE MT_CONTENT SET VIEW_COUNT=VIEW_COUNT+1 WHERE CONTENT_ID = ?;");
        mysqli_stmt_bind_param($stmt, 'i', $contentid);
        mysqli_stmt_execute($stmt);
		...
		if($contenttype == 'I') {
			...
			<table>
				...
				<tr> 
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label for="views"> <strong>Views:</strong> </label>
					<?php echo $contentcount; ?>
					&nbsp;
				...
				<?php
				if($setallfeatures) {
					$_SESSION['contentowneruploads'] = $contentowner;
					echo "<tr>"; 
					if($username != $contentowner) {
	...
	<!-- 				<input type="submit" name="download" value="Download"/> -->
						<select name="privacy" id="privacy"> 
 							<option value="0">Change Privacy</option> 
					...
					<tr> <td> <font size="5"> <strong>Uploads </strong> </font> </td> </tr>
					<tr> </tr>
					<tr> </tr>
					<tr> </tr>
					<tr> </tr>
					...
					if(isset($othersprofile)) {
						for($i=0;$i<count($contentids);$i++) {
							echo "<tr><td><a href='./userUploadsView.php?content_id=".$contentids[$i]."&media_type=".$mediatype."&others_profile=".$othersprofile."'><img src='fileUploads/image/photo.jpg' height=90 width=170/></a></td>";
							echo "<td><a href='./userUploadsView.php?content_id=".$contentids[$i]."&media_type=".$mediatype."&others_profile=".$othersprofile."' class='stylish-link'><font size='4'>$contenttitles[$i]</font></a></td></tr>";
						}
					} else {
						for($i=0;$i<count($contentids);$i++) {
			...
			if(isset($_POST['change'])) {
				...
				if($privacy == 0) {
					echo '<script type="text/javascript">';
					echo 'alert("Please select a setting")';
					echo '</script>';
				} else {
                    $stmt = mysqli_prepare($dbconnection, "UPDATE MT_CONTENT SET CONTENT_SHARING = ?
                                                           WHERE CONTENT_ID = ?");
                    mysqli_stmt_bind_param($stmt, 'si', $filePrivacy, $contentid);
                    mysqli_stmt_execute($stmt);
                    $updateprivacy = mysqli_stmt_get_result($stmt) or die("Failed to update file privacy");
                    $updateprivacyid = mysqli_insert_id($dbconnection);
                    mysqli_stmt_close($stmt);

					if(isset($updateprivacyid)) {
						echo '<script type="text/javascript">';
						echo 'alert("Done")';
						echo '</script>';
					}
